  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-ink/40 backdrop-blur-sm p-4">
      <div className="glass max-w-md w-full p-7 shadow-pop">
        <div className="text-[11px] font-bold tracking-[0.14em] text-mute">
          ШАГ {step + 1} ИЗ 3
        </div>
        ...
        {step === 1 && (
          <div className="mt-3">
            ...
            <p className="text-[13.5px] text-mute mt-1.5 leading-relaxed">
              {isDesktop() ? (
                <>
                  В приложении доступно <b>локальное распознавание</b> (whisper.cpp, офлайн).
                  Настрой его в «Настройки → Распознавание»: скачай модель одной кнопкой и укажи
                  бинарь whisper. До этого голосовой ввод требует интернет.
                </>
              ) : (
                <>
                  В браузере используется встроенное распознавание Chrome — нужен интернет.
                  В <b>десктоп-приложении</b> доступно полностью офлайн-распознавание.
                </>
              )}
            </p>
            <button onClick={() => setStep(2)} className="mt-4 w-full rounded-xl bg-ink text-paper py-3 text-[13.5px] font-bold">
              Понятно
            </button>
          </div>
        )}
        ...
        <div className="mt-5 flex items-center justify-between">
          ...
          {step < 2 && (
            <button onClick={() => setStep((s) => s + 1)} className="text-[12.5px] font-semibold text-mute hover:text-ink">
              Пропустить →
            </button>
          )}
        </div>
