  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto">
      ...
      <div className="rounded-3xl glass p-5 shadow-card flex flex-wrap items-center gap-2">
        <span className="text-[11px] font-bold uppercase tracking-[0.12em] text-zinc-500 mr-2">Приложения</span>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button onClick={refresh} className="p-2.5 rounded-xl bg-white/[0.04] border border-white/[0.09] text-zinc-400 hover:text-white" title="Обновить">
          <RefreshCw className="w-4 h-4" />
        </button>
        <button
          onClick={() => { downloadFile('flow-history.jsonl', exportJSONL(), 'application/x-ndjson'); onToast('JSONL выгружен', 'success'); }}
        ...
        <button
          onClick={() => { downloadFile('flow-stats.csv', exportCSV(), 'text/csv'); onToast('CSV выгружен', 'success'); }}
          className="flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-[12px] font-semibold bg-white/[0.04] border border-white/[0.09] hover:border-white/20"
        >
        ...
        <button
          onClick={() => {
            if (window.confirm('Очистить всю историю реплик?')) { clearJournal(); refresh(); onToast('История очищена', 'info'); }
          }}
      ...
      <div className="space-y-2">
        {records.slice(0, 200).map((r) => (
          <div key={r.id} className="flex items-center justify-between gap-3 p-3.5 rounded-2xl bg-white/[0.03] border border-white/[0.05]">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button onClick={() => reinsert(r)} title="Вставить заново (M-07)" className="p-2 rounded-lg bg-white/[0.05] border border-white/[0.08] text-zinc-400 hover:text-white">
                <RotateCcw className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => copyOne(r)} title="Копировать (M-08)" className="p-2 rounded-lg bg-white/[0.05] border border-white/[0.08] text-zinc-400 hover:text-white">
                <Copy className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => { deleteUtterance(r.id); refresh(); }} title="Удалить (M-09)" className="p-2 rounded-lg bg-white/[0.05] border border-white/[0.08] text-zinc-500 hover:text-red-400">
