...
function InsertDiagCard() {
  ...
  return (
    <div className="rounded-3xl glass p-6 shadow-card">
      ...
      <p className="text-[12px] text-mute mt-1 mb-4 leading-relaxed">
        1. Нажми кнопку. 2. В течение 8 секунд кликни мышкой в Блокнот. 3. Мы вставим три маркера тремя
        способами (печать буквами, команда окну, Ctrl+V) — по отчёту ниже видно, какой способ живёт на твоей
        машине.
      </p>
      <button
        onClick={run}
        ...
        <div className="mt-3 space-y-1.5">
          ...
          {res.length > 0 && res.every((r) => !r.ok) && (
            <p className="text-[11px] text-mute">
              ❌ у обоих? Целевое окно запущено от администратора (Windows блокирует ввод) — запусти его
              обычным способом.
            </p>
          )}
          {res.some((r) => r.ok) && (
            <p className="text-[11px] text-mute">
              ✅ живой способ будет использоваться автоматически при диктовке. Проверь, что маркеры появились
              в Блокноте.
            </p>
          )}
        </div>
