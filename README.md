...
| 🔊 **Звуки интерфейса** | Фирменные блипы | WebAudio-осциллятор, без ассетов; громкость — настройкой (AG-16) |
| 🔒 **Шифрование журнала (M-17)** | Приватность+ | Реплики на диске AES-GCM-256, пароль не хранится нигде, разблокировка фразой |
| 🗣 **Проверка вслух (AK)** | Оригинальная фича | После вставки результат перечитывается голосом — проверка без рук |
| 🧾 **Точные структуры (AO)** | Про-диктовка | Версии («три точка два точка один бета два» → 3.2.1-beta2), адрес:порт, формулы, величины (200 мс, 44 кГц, 10 МБ) |
| ⬇️ **whisper в 1 клик** | Офлайн из коробки | Официальный бинарь + модель по SHA-256, без сборки и ручных путей |
...
5. `Esc` — экстренная остановка, трей-иконка — полный контроль без окна.
...
├── src/
│   ├── App.jsx                  # оркестратор: запись, статистика, тосты
│   ├── components/
...
## 🔎 Прогон чекера локально (W-10/T-16)

Одна команда проверяет всё, что проверяет CI, и печатает сводку прогона:

```bash
npm run check
```

Внутри: `eslint --max-warnings 0` (AN-01/07/09) → все автотесты (A-13/AB) →
`prettier --check` (AN-02/14) → сводка журнала (T-15). Отдельно:
`npm run bench` — таблица замеров (X-группа), `npm run coverage` — покрытие (AN-06),
`npm run sbom` — SBOM CycloneDX (Z-06).
...
## 🔒 Приватность: шифрование журнала (M-17)

Настройки → Поведение → «Шифровать журнал»: все тексты реплик перезаписываются
шифрованными (AES-GCM-256, ключ из пароля через PBKDF2-SHA256, 210 000 итераций).
Пароль **не хранится нигде** — только соль и проверочная строка; ключ живёт в памяти
сессии. После перезапуска истории открывается по фразе (замок во вкладке «История»).
Единственная команда матрицы с реализованным M-17.

## ⚠️ Заметки

- Распознавание речи — Web Speech API: **Chrome / Edge** (в Firefox и Safari не заработает — приложение подскажет); офлайн-путь — whisper.cpp.
...
| `triggerMode` | Настройки → Клавиши | `toggle` | переключатель или удержание (AM-01; в десктопе глобальный хоткей — тогл) |
| `insertDelayMs` | settings.json | `0` | пауза перед вставкой 0..2000мс — медленным приложениям (AM-20) |
| `micDeviceId` | Настройки → Микрофон | `''` | устройство ввода, применяется к следующей реплике (C-02) |
| `noiseSuppression` | Настройки → Поведение | ✅ | шумоподавление на входе (C-16) |
| `soundVolume` | Настройки → Поведение | `100%` | громкость сигналов (AG-16) |
| `vadThreshold` | settings.json | `0.01` | порог VAD: обрезка тишины (E-01/E-02/E-04) |
| `autostart` | Настройки → Фон | ⬜ | автозапуск при входе в систему (B-10) |
...
Распознаватель — сменный блок. Три пути (переключаются настройкой `asrProvider`):
| Режим | Что происходит | Сеть |
|---|---|---|
| `auto` (по умолчанию) | Web Speech + при его отказе локальный whisper, затем Gemini | частично |
| `whisper` | **только локальный** whisper.cpp: ни байта наружу | нет |
| `gemini` | только Gemini с фолбэк-цепочкой моделей | да |
