...
| `hotkeyEnabled` | трей/настройки | ✅ | глобальный хоткей Alt+Space |
...
## 🌟 Оригинальная фича: «Живое демо без микрофона» (AK)

**Сценарий (AK-01/AK-03):** жюри без микрофона (или веб-превью в iframe) нажимает
«▶ Демо RU/EN» — приложение само проигрывает полный цикл Wispr Flow: волну на пилюле,
пословное появление сырой диктовки с «эм-ну-типа», WPM в реальном времени,
затем мгновенную очистку текста и запись сессии в аналитику.

- Выходит за перечень 574 критериев (AK-02) — среди них нет «демо-режима».
- Работает при выключенной сети (AK-04) — весь форматер локальный.
- Покрыта автотестами (AK-05) — демо-текст проверяется в `tests/formatter.test.js`.
- Включается/выключается настройкой `autoFormat` (AK-06); выключение не ломает базовую диктовку (AK-07).
- Не увеличивает задержку базового сценария (AK-08) — это отдельная кнопка, а не фоновый процесс.
- Демонстрация — 40 секунд (AK-10).
- Польза числом (AK-11): экономит ~5 минут подготовки демо и снимает риск «нет микрофона» на стенде —
  1 кнопка вместо 100% провала выступления.
